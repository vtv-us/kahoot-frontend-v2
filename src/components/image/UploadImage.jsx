...
function ImageUpload(props) {
  ...
  return (
    <label
      className={`cursor-pointer flex items-center justify-center bg-gray-100 border border-dashed w-[185px] min-h-[185px] rounded-lg ${className} relative overflow-hidden group`}
    >
      <input
        type="file"
        name={name}
      ...
      {progress !== 0 && progress !== 100 && (
        <div className="loading w-16 h-16 border-8 border-green-500 border-t-transparent animate-spin absolute z-10 rounded-full" />
      )}
      {!image && progress === 0 && (
